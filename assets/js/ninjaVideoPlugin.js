    ...
    for (var c = this.getLis(), b = 0; b < c.length; b++)
        for (var e = c[b].getElementsByTagName("*"), d = 0; d < e.length; d++)
            if (e[d].className == "video") {
                ...
                var g = this;
                if (c[b].player) {
                    c[b].onclick = function() { g.playVideo && g.playVideo(this.player) };
                    if (c[b].player.tp != "VA") {
        ...
        switch (a.tp) {
            case "VM":
                if (this.playVideo_VM) this.playVideo_VM(a);
                    ...
                    var c = this;
                    setTimeout(function() { c.playVideo(a, ++b) }, 500)
                } else this.next();
                break;
            case "YT":
...
function set_ZIndex(a, b) { a.parentNode.style.zIndex = b ? "2147481964" : "auto" }
...
function extendSliderFunForVimeo() {
    this.postToIframe = function(a, d, b) {
        ...
        if (window.JSON) {
            var c = { method: d };
            if (b) c.value = b;
            this.iframe.contentWindow.postMessage(JSON.stringify(c), a.getAttribute("src").split("?")[0])
        }
    };
    this.playVideo_VM = function(a) {
        if (LIS_VMs.Rdy) this.postToIframe(a, "play");
            ...
            var b = this;
            setTimeout(function() { b.postToIframe(a, "play") }, 300)
        }
    };
    this.unloadPlayer_VM = function(a) { window.JSON && a.contentWindow.postMessage(JSON.stringify({ method: "pause" }), a.getAttribute("src").split("?")[0]) };
    this.backToStartFrame = function() { if (this.iframe && window.JSON) this.iframe.src = this.iframe.getAttribute("src") }
}
...
function addVimeoEventHandlers(a) {
    if (LIS_VMs.length) {
        ...
        function b(d, a) { if (window.JSON) { var e = { method: d }; if (a) e.value = a; for (var b = 0; b < LIS_VMs.length; b++) c(d, a, LIS_VMs[b].player) } }

        function c(d, b, a) {
            var c = { method: d };
            if (b) c.value = b;
            a.contentWindow.postMessage(JSON.stringify(c), a.getAttribute("src").split("?")[0])
        }

        function g() { setSliderWhenOnPlay(a) }

        function e() {
            if (a.iframe == a.getLis()[a.getIndex()].player) {
                set_ZIndex(a.iframe, 0);
                a.backToStartFrame();
                setTimeout(function() { a.next() }, 120)
            }
        }
    }
}
...
function addYoutubeEventHandlers(c) {
    if (LIS_YTs.length) {
        ...
        for (var a = 0; a < LIS_YTs.length; a++) LIS_YTs[a].player.src += "&origin=" + location.protocol + "//" + location.hostname + (location.port ? ":" + location.port : "")
    }
}
...
function onYouTubePlayerAPIReady() {
    ...
    _zSlider.playVideo_YT = function(a) {
        if (a.parentNode.parentNode.yplayer)
            if (typeof a.parentNode.parentNode.yplayer.playVideo == "function") a.parentNode.parentNode.yplayer.playVideo();
            else setTimeout(function() { _zSlider.playVideo_YT(a) }, 100)
    }
}

function extendSliderFunForhtml5VA() {
    this.playVideo_VA = function(a) { a.play && a.play() };
    this.unloadPlayer_VA = function(a) { a.pause && a.pause() }
}
...
function addVAEventHandlers(c, a) {
    ...
    var b = {
        ...
        play: function() { setSliderWhenOnPlay(c) },
        ended: function() {
            set_ZIndex(a, 0);
            c.next()
        },
